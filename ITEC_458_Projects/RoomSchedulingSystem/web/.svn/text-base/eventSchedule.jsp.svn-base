...
<tiles:insert page="/commonLayout/commonLayout.jsp" flush="true">
    ...
    <tiles:put name="content" direct="true">
        ...
        <html:errors />
        <html:form action="eventSchedule">
                    ...
                    <tr>
                        ...
                        <c:forEach var="obj" items="${objs}" varStatus="objIndex">
                            <td>
                                ...
                                <c:set target="${isEmpty}" property="isOwner" value="false"/>
                                <c:forEach var="eventSchedule" items="${period.eventSchedules}">
                                    <c:if test="${eventSchedule.dayOfWeek == (objIndex.count) && eventSchedule.room.id == roomObj.id && eventSchedule.semesterWeek == semesterWeek}">
                                        <div style="width:100%;height:100%" id="${eventSchedule.reservationStatus}">
                                        ...
                                        <c:set target="${isEmpty}" property="eventSchedule" value="${eventSchedule.id}"/>
                                ...
                                %>
                                    <c:if test="${isEmpty['isEmpty'] == 'false' && isEmpty['isOwner'] == 'true'}">
                                        <html:checkbox  property="roomEventSchedule" value="${isEmpty['eventSchedule']}" styleClass="notvisiable"/>
                                    </c:if>
                                <%
                                }
                                %>
            ...
            if (user.hasPermission("Remove")) {
                ...
                <%--html:select property="selectedEventType" value="id">
                    <c:forEach var="e" items="${eventType}">
                        <html:option value="${e}">${e}</html:option>
                    </c:forEach>
                    <html:optionsCollection name="eventType" label="name" value="value"/>
                </html:select--%>
                <html:select property="selectedEvent" value="id">
                    <html:optionsCollection name="events" label="name" value="id" />
                </html:select>
            </p>
        </html:form>
    </tiles:put>
