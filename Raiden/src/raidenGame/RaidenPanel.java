...
public class RaidenPanel extends JPanel implements KeyListener, Runnable {
  ...
  private RaidenGame rgTop; //Main Frame
  ...
  //my changes
  //private int recX,recY,recW,recH,speed;
  ...
  private Player player;
    ...
    public RaidenPanel(RaidenGame rg, int period){
        rgTop = rg;
    this.period = period;
...
//    }
 ...
 private void readyForTermination()
  ...
  { if (!isPaused && !gameOver){
        player.update();
        if (key[KeyEvent.VK_UP]) player.move(Player.DIRECTION.UP);//recY -= speed;
        if (key[KeyEvent.VK_DOWN]) player.move(Player.DIRECTION.DOWN);
        if (key[KeyEvent.VK_LEFT]) player.move(Player.DIRECTION.LEFT);
        if (key[KeyEvent.VK_RIGHT]) player.move(Player.DIRECTION.RIGHT);
  ...
  private void gameRender()
  {
	...
	dbg.setColor(Color.black);
        //dbg.fillRect(recX,recY,recW,recH);
        player.draw(dbg);
//        for (int i = 0; i < bullets.length; i++) {
//            if(bullets[i] != null){
//                
//                bullets[i].drawAndMoveBullet(dbg);
//            }
//        }
        player.drawBullets(dbg);
    if (gameOver)
      gameOverMessage(dbg);
  }  // end of gameRender()
